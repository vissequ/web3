...
contract Log3 {
    struct Post {
        uint id;
        string header;         // Replaced title with header
        string message;
        address creator;
        uint timestamp;
        uint likes;
        uint dislikes;
        bool hasPaywall;       // Paywall for this post
        uint paywallAmount;    // Paywall amount in smallest POL units
    }
    ...
    function createPost(
        ...
        uint amountInPOL = _paywallAmount * 10**18; // Convert to smallest POL units
        if (_hasPaywall) {
            require(msg.value >= amountInPOL, "Paywall enforced. Insufficient funds.");
        }
        ...
        emit PostCreated(postCount, _header, _message, msg.sender, block.timestamp, _hasPaywall, amountInPOL);
    }
